...
async def test_buy_reduces_cash_and_creates_holding(test_user_and_token):
    ...
    async with api_client() as client:
        response = await client.post(
            "/trading/buy", json={"propertyId": str(property_result.inserted_id)}, headers=headers
        )
        ...
        trade = await db.trades.find_one({"propertyId": property_result.inserted_id, "side": "buy"})
...
async def test_sell_creates_trade_and_removes_holding(test_user_and_token):
    ...
    await db.marketindex.insert_one(
        {
            ...
            "policy": 0.0,
            "locals": [
                {"zone": "Ixelles", "access": 0.0, "attract": 0.0, "nuisance": 0.0, "tension": 0.0}
            ],
        }
    )
    ...
    await db.pricehistory.insert_one(
        {"propertyId": property_result.inserted_id, "t": "2020-2", "price": current_price}
    )

    # Test: Sell property with authentication
    async with api_client() as client:
        response = await client.post(
            "/trading/sell", json={"propertyId": str(property_result.inserted_id)}, headers=headers
        )
