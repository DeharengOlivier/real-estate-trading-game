...
async def test_the_summary_costs_the_same_whatever_the_portfolio_holds(
    ...
    assert counted_queries.count() <= 8, (
        f"{counted_queries.count()} round trips for {HOLDINGS} holdings: {counted_queries.calls}"
    )
...
async def test_the_summary_reads_no_document_per_renovation_work(
    ...
    assert counted_queries.count("renovations.find_one") == 0, (
        "one lookup per work is the shape that made this page grow with the portfolio"
    )
...
@pytest.mark.asyncio
async def test_the_bound_holds_when_the_portfolio_doubles(test_user_and_token, counted_queries):
