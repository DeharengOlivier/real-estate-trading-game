...
def _market_index(zone="Ixelles"):
    return {
        ...
        "confidence": 0.0,
        "policy": 0.0,
        "locals": [{"zone": zone, "access": 0.0, "attract": 0.0, "nuisance": 0.05, "tension": 0.0}],
    }
...
def test_higher_quality_increases_price():
    ...
    mi = _market_index()
    assert services.compute_property_price(better, mi) > services.compute_property_price(base, mi)
...
async def test_batched_prices_handle_an_empty_list_and_duplicates():
    ...
    prices = await get_property_current_prices(db, [prop.inserted_id, prop.inserted_id], "2020-1")
